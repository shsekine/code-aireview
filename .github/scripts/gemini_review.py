#!/usr/bin/env python3
"""GitHub Actions から呼ばれる Google Gemini コードレビュースクリプト。

`claude_review.py` の Gemini 版。プロンプトと diff 解析ロジックは共通で、
LLM 呼び出し部分のみ Gemini API に差し替えてある。
...
def env(name: str, required: bool = True, default: str | None = None) -> str:
    ...
    if required and not val:
        sys.exit(f"[gemini-review] required env var missing: {name}")
    return val or ""
...
def run(cmd: list[str]) -> str:
    ...
    if result.returncode != 0:
        sys.exit(
            f"[gemini-review] command failed: {' '.join(cmd)}\n"
...
def extract_json(text: str) -> dict[str, Any]:
    ...
    if start == -1 or end == -1 or end < start:
        raise ValueError(f"no JSON object in Gemini response: {text[:300]}")
...
def validate_comments(
    ...
    for c in comments:
        ...
        if not path or not isinstance(line, int) or not body:
            print(f"[gemini-review] skip malformed comment: {c}")
            continue
        if path not in by_path:
            print(f"[gemini-review] skip comment for unknown file: {path}")
            continue

        target = by_path[path]
        allowed = target.right_lines if side == "RIGHT" else target.left_lines
        if line not in allowed:
            print(f"[gemini-review] skip out-of-diff comment: {path}:{line} ({side})")
            continue
...
def post_review(
    ...
    if r.status_code >= 300:
        print(f"[gemini-review] review API error {r.status_code}: {r.text[:500]}")
        # インラインコメントが全部弾かれた場合、本文のみで再送
        if comments:
            print("[gemini-review] retrying without inline comments...")
            payload["comments"] = []
        ...
        print(f"[gemini-review] review posted: {r.json().get('html_url')}")
...
def main() -> int:
    ...
    print(f"[gemini-review] {repo}#{pr_number}  base={base_sha[:7]} head={head_sha[:7]}")
    ...
    if not target_files:
        print("[gemini-review] no Python/TS/JS files changed, skipping")
        return 0

    # Gemini には対象ファイルだけの diff を渡す(node_modules 等のノイズ除去)
    ...
    print(f"[gemini-review] calling Gemini ({model}), diff={len(focused_diff)} chars, "
          f"files={len(target_files)}")
    raw_response = call_gemini(model, SYSTEM_PROMPT, user_prompt)

    try:
        parsed = extract_json(raw_response)
    except (ValueError, json.JSONDecodeError) as e:
        print(f"[gemini-review] failed to parse Gemini response: {e}")
        # フォールバック: 全文を overall コメントとして投稿
        post_review(
            repo, pr_number, head_sha, token,
            body=f"## Gemini AI Code Review (raw output)\n\n{raw_response}",
            comments=[],
        )
        return 0
